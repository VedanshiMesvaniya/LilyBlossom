...
export function AdminDashboardPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const firstError = [series, movies, airing, upcoming, pendingAnnouncements].find((result) => result.error);
      if (firstError) {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10 font-ui">
      ...
      {error ? (
        <p className="text-sm text-red-500">Could not load dashboard stats: {error}</p>
      ) : (
